...
def my_parse_date(x):
    if x:
        return dateutil.parser.parse(x, dayfirst=True).isoformat()
    return x
...
def parse_all(harvested_data):
    ...
    set_objects(parsed_data, "clinical-trials", "euctr.json.gz")
    return {"status": "ok", "source": "euctr", "nb_studies_harvested": len(harvested_data), "nb_studies_parsed": len(parsed_data)}

def harvest_parse_euctr():
...
def harvest():
    ...
    nb_pages = int(re.sub(".*Displaying page 1 of ", "", soup.find(class_="outcome").get_text().replace("\n"," ")).split(".")[0])
    ...
    for p in range(1, nb_pages + 1):
        ...
        #print("{} : {}".format(p, len(current_links)))
...
def parse_results(eudract):
    ...
    res = {}
    res['results_first_submit_date'] = my_parse_date(infos.get('First version publication date'))
    res['study_start_date'] = my_parse_date(infos.get('Actual start date of recruitment'))
...
def parse_euctr(html):
    ...
    res = {}
    res['eudraCT'] = infos.get("A.2;EudraCT number")
    res['has_results'] = summary_infos.get('Trial results') == "View results"
    if res['has_results']:
    ...
    res['study_first_submit_date'] = my_parse_date(summary_infos.get('Date on which this record was first entered in the EudraCT database'))
    res['study_completion_date'] = my_parse_date(infos.get("P.;Date of the global end of the trial"))
