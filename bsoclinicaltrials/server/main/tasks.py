...
def create_task_transform_load(args: dict) -> dict:
    ...
    if args.get('harvest', True):
        res_ct = harvest_parse_clinical_trials(to_harvest=True, to_parse=True, harvest_date = harvest_date_ct)
        res_euctr = harvest_parse_euctr(to_harvest=True, to_parse=True, harvest_date = harvest_date_euctr)
